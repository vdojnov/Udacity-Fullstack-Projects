...
class Venue(db.Model):
    ...
    def get_venue(self):
        ...
        'id': self.id,
        'name': self.name,
        'num_upcoming_shows': Show.query.filter(Show.venue_id == self.id, Show.start_time > datetime.now()).count()
        }
        return data
...
def search_artists():
  ...
  #
  # sun = request.form.get('search_term', '')
  #  response={
  #  "count": Artist.query.filter(Artist.name.ilike('%viktor%')).count(),
  #  "data": artist.query.all()
  #  }
